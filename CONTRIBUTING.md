...
Read [`MISSION.md`](MISSION.md) before making product or architecture changes.
It defines Cab's product bet, interaction modes, command model, and architectural
principles.

Changes should move Cab toward that mission. Passing tests is required, but it
is not the whole question. Ask whether the change makes the human-agent interface
loop clearer, safer, more observable, or more capable.
